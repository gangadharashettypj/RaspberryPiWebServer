...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    companion object {
        ...
        private const val MAX_FONT_SIZE = 46F
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webPage.loadUrl("http://" + ipAddress.text.toString() + ":3400")
        webPage.reload()
    }
    ...
    private fun httpGet(): Bitmap? {

        return Glide.with(applicationContext).asBitmap()
            .load("http://" + ipAddress.text.toString() + ":3400/video/capture")
            .into(100, 100).get()
    }
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.saveIp -> {
                ...
                getSharedPreferences("IPPPP", Context.MODE_PRIVATE).edit()
                    .putString("IP", ipAddress.text.toString()).apply()
                webPage.loadUrl("http://" + ipAddress.text.toString() + ":3400")
                webPage.reload()
            }
        }
    ...
    private fun runObjectDetection(bitmap: Bitmap) {
        ...
        val resultToDisplay = results.map {
            ...
            val title = when(category.label){
                "healthy" -> "Heealthy"
                "pestalotiopsis" -> "Anthracnose"
                "cordana" -> "cordana"
                "sigatoka" -> "Black Sigatoka"
                else -> ""
            }
            val description = when(category.label){
                "healthy" -> ""
                "pestalotiopsis" -> "Spraying of chlorothanoil (0.2%) and Bavistin (1%) 4 times at 15 days interval"
                "cordana" -> "5g/l of Naclo3, NaHCo3 & 5g/l Cacl2"
                "sigatoka" -> "Pesticide: Thlophanate metryl (1g / 1 Litre of H2O) or Thlophanate methyle (1g / 1 liter of H2O"
                else -> ""
            }
            runOnUiThread {
